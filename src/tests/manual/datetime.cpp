...
QDateTime ParseDateTime(const QString &str,bool *ok)
{
  if(str.trimmed().contains(" ")) {
    return ParseRfc822DateTime(str,ok);
  }
  return ParseXmlDateTime(str,ok);
}
...
// XML xs:time format
//
QTime ParseXmlTime(const QString &str,bool *ok,int *day_offset)
{
  QTime ret;
  QStringList f0;
  QStringList f1;
...
QDateTime ParseRfc822DateTime(const QString &str,bool *ok)
{
  ...
  return QDateTime(date,time).addSecs(tz_offset-TimeZoneOffset());
}
